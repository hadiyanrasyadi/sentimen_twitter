...
def main():
	...
	with open(fileLatih+".json",'r') as f,  open('Hasil dan pembahasan.txt', 'w') as cekcek, open('Hasil Informative Feature.txt', 'w') as file, open(fileLatih+'_hasil_normalisasi.txt','w') as hasil_norm, open(fileLatih+'_hasil_stemming.txt','w') as hasil_stem, open(fileLatih+"_hasil_stopword.txt",'w') as hasil_stop, open(fileLatih+"_hasil_praproses.txt",'w') as hasil_tweet,open('key_norm.csv') as filecsv, open('stopword_list_TALA.txt','r') as stp_file, open(fileUji+".json",'r') as fOpen, open(fileUji+"_hasil_akurasi.txt",'w') as hasil_akurasi, open('my_classifier.pickle', 'wb') as model, open('clean_tweet.json', 'w') as decode:
		...
		cekcek.write('\n\n\n ################################################## selesai tulis dawa awal\n\n\n')
		cekcek.write(']')

		print('Donee menulis data awal ^^')

		print('Membersihkan karakter2 khusus seperti link web, akun user')
		...
		print ('menulis pembersihan data')
		#menulis hasil tweet
		cekcek.write('[\n')
		FNL= False
		for i in clean_regular:
			if FNL == True:
				cekcek.write(',\n')
			FNL = True
			cekcek.write('"('+str(i)+')"')

		cekcek.write('\n\n\n ################################################## selesai tulis pembersihan data\n\n\n')
		cekcek.write(']')
		...
		print ('menulis Hasil Normalisasi dan menghilangkan angka')
		#menulis hasil tweet
		cekcek.write('[\n')
		FNL= False
		for i in clean_norm:
		...
		cekcek.write('\n\n\n ################################################## selesai tulis Hasil Normalisasi dan menghilangkan angka\n\n\n')
		cekcek.write(']')
